...
const onSortClink = (props, event) =>
    props.callback(sortColumn(props.column.name));
...
export default props => (
    <div style={getColumnStyle(props.column, props.index, props.ghost)}>
        <div style={{ display: 'flex', alignItems: 'center', height: 30, }} onClick={linkEvent(props, onSortClink)}>
        ...
        {props.column.enableFiltering &&
            <input style={getInputStyle()}
                ...
                onInput={linkEvent(props, onFilterInput)} />
        }
    </div>
);
